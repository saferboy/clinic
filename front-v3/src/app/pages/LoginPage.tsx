...
export function LoginPage() {
  const [email, setEmail] = useState('admin@clinic.uz');
  ...
  const quickLogin = async (role: string) => {
    const creds: Record<string, { email: string; password: string }> = {
      admin: { email: 'admin@clinic.uz', password: '123456' },
      doctor: { email: 'nilufar@clinic.uz', password: '123456' },
      receptionist: { email: 'receptionist@clinic.uz', password: '123456' },
      accountant: { email: 'accountant@clinic.uz', password: '123456' },
    };
    const c = creds[role];
    setEmail(c.email);
    ...
    <div className="min-h-screen flex bg-white dark:bg-slate-900">
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          ...
          {/* Quick login buttons */}
          <div className="mb-6">
            <p className="text-xs text-muted-foreground mb-2">Tezkor kirish (demo):</p>
            <div className="flex flex-wrap gap-2">
              {['admin', 'doctor', 'receptionist', 'accountant'].map(role => (
                <button
                  key={role}
                  onClick={() => quickLogin(role)}
                  className="text-xs px-3 py-1.5 rounded-lg bg-muted hover:bg-accent border border-border transition-colors capitalize"
                >
                  {role === 'admin' ? 'Admin' : role === 'doctor' ? 'Shifokor' : role === 'receptionist' ? 'Registrator' : 'Buxgalter'}
                </button>
              ))}
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-border bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full px-4 py-2.5 rounded-xl border border-border bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm pr-12"
                />
                <button
                  type="button"
          ...
          <p className="mt-6 text-center text-xs text-muted-foreground">
            Demo parol: <span className="font-medium text-foreground">123456</span> (barcha foydalanuvchilar uchun)
          </p>
        </div>
      </div>
    </div>
