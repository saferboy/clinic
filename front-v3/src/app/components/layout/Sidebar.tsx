...
export function Sidebar({ collapsed, onToggle, mobileOpen, onMobileClose }: SidebarProps) {
  ...
  const filteredNav = navItems.filter(item =>
    currentUser ? hasAccess(currentUser.role, item.page) : false
  );
      ...
      {currentUser && (
        <div className={`p-3 border-t border-slate-700/50 ${collapsed ? '' : ''}`}>
          <div className={`flex items-center gap-3 mb-2 ${collapsed ? 'justify-center' : ''}`}>
            <div className={`w-8 h-8 rounded-full ${roleColors[currentUser.role] || 'bg-slate-600'} flex items-center justify-center flex-shrink-0 text-white text-xs font-semibold`}>
              {currentUser.name.charAt(0)}
            </div>
            {!collapsed && (
              <div className="min-w-0">
                <div className="text-white text-xs font-medium truncate">{currentUser.name}</div>
                <div className="text-slate-400 text-xs">{getStatusLabel(currentUser.role)}</div>
              </div>
            )}
          </div>
          <button
